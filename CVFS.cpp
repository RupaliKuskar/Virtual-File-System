...
#include<string.h>               //for inbuilt string function
#include<unistd.h>               // For macro
#include<iostream>
#include<io.h>

#define MAXINODE 50             //Maximum files

#define READ 1                   //Permission(eg.read,write)
#define WRITE 2
...
#define START 0                   //For handling lseek function
#define CURRENT 1
#define END 2

typedef struct superblock                 //structure of superblock
{
    int TotalInodes;
    int FreeInode;
...
{
    char FileName[MAXINODE];                   //Practically it should not be, it must be in directory
    int InodeNumber;
    int FileSize;                        
    int FileActualSize;                  // Size of file
    int FileType;                        //text file
    char *Buffer;                        //Pointing to block(data)
    int LinkCount;                       //eg.shortcut(1) 
    int ReferenceCount;                  //number of references that refering the  document(eg. TV )
    int permission;                      // 1 2 3(create)
    struct inode *next;                  //self referential structure
}INODE,*PINODE,**PPINODE;

typedef struct filetable                  //structure of filetable
{
    int readoffset;                       //till 
    int writeoffset;
    int count;                            //1
    int mode;                            // 1 Read 2  Write 3 Read & Write (read)
    PINODE ptrinode;
}FILETABLE,*PFILETABLE;

typedef struct ufdt                      //structure of ufdt(user file description table )
{
    PFILETABLE ptrfiletable;
}UFDT;

UFDT UFDTArr[MAXINODE];                      //Array of UFDT structure(index of fd)
...
void CreateDILB()
{
    int i = 1;               //initialization of index
    PINODE newn = NULL;                      
    PINODE temp = head;
...
int main()
{
    ...
    InitiaseSuperBlock();               //Auxilary data(Data before run)
    CreateDILB(); 

    while(1)                           //Unconditional loop
    {
        fflush(stdin);                  //for clearing previous data
        strcpy(str,"");
        ...
        else if(count == 2)
        {
            ...
            else if(strcmp(command[0],"write") == 0)           //write
            {
                fd = GetFDFromName(command[1]);                // condition handle
                if(fd == -1)
                {
                    printf("Error : Incorrect parameter\n");
                    continue;
